...
import { arrowDownCircle, arrowUpCircle, bag, checkmarkCircleOutline, closeCircleOutline, createOutline, timeOutline, trashOutline } from "ionicons/icons";
  ...
  return <IonPage><IonHeader><IonToolbar><IonButtons slot="start"><IonMenuButton /></IonButtons><IonTitle>Budget Definitions</IonTitle><IonButtons slot="end"><IonButton onClick={() => history.push("/budget/history")} title="Budget History"><IonIcon icon={timeOutline} /></IonButton></IonButtons></IonToolbar></IonHeader><IonContent className="ion-padding">
    ...
    <IonList>{orderedBudgets.map((budget) => {
      ...
      return <IonItem key={budget.id}><IonGrid style={{ width: "100%" }}><IonRow>
        ...
        <IonCol size="7"><h3 className="item-description"><button type="button" onClick={() => { void openOccurrences(budget); }} title="View Budget occurrences" style={{ padding: 0, border: 0, background: "none", color: "inherit", font: "inherit", cursor: "pointer", textAlign: "left" }}>{budget.description}</button>{!budget.isFlexible && <IonIcon icon={bag} style={{ marginLeft: "8px", fontSize: "1rem", color: "var(--ion-color-warning)", verticalAlign: "middle" }} title="Strict Budget" />}</h3>
