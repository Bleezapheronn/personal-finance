...
- Reports has one high-risk flag-gated read experiment; it remains read-only
  and disables the monthly chart and bucket/category drilldowns in
  `http-readonly` mode.
- Budget has one high-risk flag-gated read experiment; it remains read-only,
...
| Accounts | `VITE_PERSONAL_FINANCE_ACCOUNTS_READ_EXPERIMENT=true` | Dexie | Loads through selected-read; existing images load through the authenticated read-only image endpoint | Create, edit, activate/deactivate, and delete disabled in `http-readonly` | Passes when Account image presence also matches | Missing or invalid SQLite images use the initials fallback; transaction-derived usage checks remain on the Dexie path and are not migrated | Turn flag off or set backend to `dexie`, then restart Vite |
...
| Transactions | `VITE_PERSONAL_FINANCE_TRANSACTIONS_READ_EXPERIMENT=true` | Existing Dexie Transactions behavior | List loads through selected-read with paginated reads | Detail navigation, add, edit, delete, transfer, import, and CSV export remain guarded according to the active SQLite capabilities; Duplicate is navigation/prefill only | Transactions read parity diagnostic passes with fresh matching SQLite | High-risk workflow screen; missing or invalid SQLite account images use placeholders; writes, exports, and transfers require their separate approved paths; fresh backup/import/API restart required before trusting parity | Turn flag off or set backend to `dexie`, then restart Vite |
...
- Account/payment icons may fall back to placeholders in `http-readonly`
  ...
  path.
- Reports has a per-screen read experiment flag:
  `VITE_PERSONAL_FINANCE_REPORTS_READ_EXPERIMENT=true`.
- Default behavior remains Dexie.
- In `http-readonly` mode, the main Reports period report may load
  transaction, category, and bucket inputs through selected-read paginated
  reads capped at 5,000 transaction inputs for the selected period, but the
  monthly chart and bucket/category drilldown modal remain disabled because
  those paths still use Dexie-backed report helpers.
- The Reports experiment is high-risk and should only be trusted after a fresh
  matching SQLite baseline and passing Reports parity diagnostic.
- Truncated selected-read report inputs show a warning and must not be treated
  as full-confidence report totals.
- Budget has a per-screen read experiment flag:
  `VITE_PERSONAL_FINANCE_BUDGET_READ_EXPERIMENT=true`.
- Default behavior remains Dexie.
- In `http-readonly` mode, Budget inputs may load through the selected-read
  facade with bounded paginated reads, but add, edit, delete, completion,
  transaction linking, import, export, and load-more lifecycle actions remain
  disabled because writes, linking, and lifecycle behavior have not migrated.
- The Budget experiment is high-risk and should only be trusted after a fresh
  matching SQLite baseline and passing Budget read parity diagnostic.
- Snapshot lifecycle helpers are bypassed in `http-readonly`; generation,
...
- Accounts `http-readonly` visual parity is incomplete because account
  images/icons are unavailable or invalid in the matching SQLite database.
