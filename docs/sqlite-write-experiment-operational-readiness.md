...
| Buckets | Create/update; optional empty-and-unused delete and exact-ID merge that moves Category parents | Reorder, active-state changes, cascades, automatic/fuzzy/bulk merge, Category merging | `PERSONAL_FINANCE_ENABLE_BUCKET_CATEGORY_WRITES=true`; optional `PERSONAL_FINANCE_ENABLE_BUCKET_DELETE_MERGE_WRITES=true` | `VITE_PERSONAL_FINANCE_BUCKETS_CATEGORIES_WRITE_EXPERIMENT=true`; optional `VITE_PERSONAL_FINANCE_BUCKET_DELETE_MERGE_WRITE_EXPERIMENT=true` | Passed: focused lifecycle checks; explicit disposable mutation smoke required per database | Create/update passed; lifecycle browser pass remains required | Discard or re-import SQLite; rotate an authoritative checkpoint after lifecycle writes; Dexie and financial rows are unaffected |
...
- Bucket reorder and unsupported Bucket/Category active-state changes remain
  unavailable. Optional Bucket deletion is empty-and-unused only; exact-ID
  Bucket merge changes only Category parents, requires matching activity and
  two report-visible Buckets, preserves Category IDs, and never merges
  duplicate names. Category deletion is unused-only; same-Bucket exact-ID
